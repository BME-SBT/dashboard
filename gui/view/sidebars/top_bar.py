...
from gui.view.panel_elements.networkstatus_widget import NetworkstatusImage
...
class TopBar(QWidget):
    ...
    def __init__(self):
        ...
        self.motor_data_warn = WarnImage(
            0b00001010010,
            "motor_data_warn",
            lambda v, name: False,
            lambda s, os: s == SensorState.MISSING_DATA,
        )
        self.motor_temp_warn = WarnImage(
            0b00010110010, "motor_temp_warn", lambda v, name: v >= 65.0
        )
        self.motor_temp_err = WarnImage(
            0b00010110010, "motor_temp_err", lambda v, name: v >= 85.0
        )
        self.controller_temp_warn = WarnImage(
            0b00011010010, "controller_temp_warn", lambda v, name: v >= 70.0
        )
        self.battery_soc_warn = WarnImage(
            0b00001110000, "battery_soc_warn", lambda v, name: v <= 40.0
        )
        self.battery_soc_err = WarnImage(
            0b00001110000, "battery_soc_err", lambda v, name: v <= 37.2
        )
        self.battery_temp_warn = WarnImage(
        ...
        main_layout.addWidget(self.networkstatus)
